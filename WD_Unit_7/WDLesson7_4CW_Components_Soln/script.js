function init(){  
  ...
  //Task 1: Create a gallery using the images in the avengers_images array. 
   createGallery(avengers_images, "gallery_output");

  //Task 2: Create a modal to display the logo image when you click on the text Marvel Avengers!!.
  createModal("Marvel Avengers!!", "<img src='images/logo.png'>", "modal_output");
  ...
  for(let i = 0; i < avengers.length; i++){
    ...
    createFlipCard(front, back, "flipcard_output");
  }
  ...
  for(let i = 0; i < avengers.length;i++){
    ...
    createCollapsible(avenger.name,content, "collapsible_output");
  }
